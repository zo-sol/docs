...
టేబుల్‌ను స్పష్టంగా సృష్టించడానికి [`create_table()`](#create_table) ఉపయోగించండి. DbRoot PDA లో స్థలం తక్కువగా ఉంటే, SDK అదే ట్రాన్సాక్షన్‌లో దాన్ని ఆటోమేటిక్‌గా విస్తరిస్తుంది — అదనపు దశలు అవసరం లేదు.
...
- [`create_table()`](#create_table): కొత్త టేబుల్‌ను సృష్టించండి (అవసరమైతే DbRoot ఆటోమేటిక్‌గా విస్తరిస్తుంది)
- [`write_row()`](#write_row): ఇప్పటికే ఉన్న టేబుల్‌కు కొత్త వరుసను జోడించండి
...
వాలెట్ సంతకం నుండి [`derive_x25519_keypair()`](#derive_x25519_keypair) ద్వారా X25519 కీ-పెయిర్ డెరైవ్ చేయబడుతుంది.

#### సంబంధిత కీలక ఫంక్షన్లు

- [`derive_x25519_keypair()`](#derive_x25519_keypair): వాలెట్ సంతకం నుండి ఎన్‌క్రిప్షన్ కీ-పెయిర్ డెరైవ్ చేయండి
- [`dh_encrypt()`](#dh_encrypt) / [`dh_decrypt()`](#dh_decrypt): ఒకే గ్రహీత ఎన్‌క్రిప్షన్/డీక్రిప్షన్
- [`password_encrypt()`](#password_encrypt) / [`password_decrypt()`](#password_decrypt): పాస్‌వర్డ్-ఆధారిత ఎన్‌క్రిప్షన్/డీక్రిప్షన్
- [`multi_encrypt()`](#multi_encrypt) / [`multi_decrypt()`](#multi_decrypt): బహుళ-గ్రహీత ఎన్‌క్రిప్షన్/డీక్రిప్షన్
...
| **పారామీటర్లు** | `connection`: AsyncClient<br/>`signer`: Keypair లేదా WalletSigner<br/>`db_root_id`: డేటాబేస్ ID (bytes లేదా str)<br/>`connection_seed`: కనెక్షన్ సీడ్ (bytes లేదా str)<br/>`row_json`: JSON డేటా (str) |
|----------------|--------------------------|
| **రిటర్న్స్** | లావాదేవీ సంతకం (str) |
...
| **పారామీటర్లు** | `connection`: AsyncClient<br>`signer`: Keypair<br>`db_root_id`: డేటాబేస్ ID (bytes లేదా str)<br>`meta`: నిల్వ చేయవలసిన మెటాడేటా (bytes లేదా str) |
|----------|--------------------------|
| **రిటర్న్** | ట్రాన్సాక్షన్ సిగ్నేచర్ (str) |
...
| **పారామీటర్లు** | `connection`: AsyncClient<br/>`signer`: Keypair లేదా WalletSigner<br/>`db_root_id`: డేటాబేస్ ID (bytes లేదా str)<br/>`table_seed`: టేబుల్ సీడ్ (bytes లేదా str)<br/>`table_name`: టేబుల్ పేరు (bytes లేదా str)<br/>`column_names`: కాలమ్ జాబితా (list[bytes లేదా str])<br/>`id_col`: ID కాలమ్ (bytes లేదా str)<br/>`ext_keys`: ఎక్స్‌టెన్షన్ కీలు (list[bytes లేదా str])<br/>`gate_mint`: ఐచ్చిక టోకెన్ గేట్ (Pubkey)<br/>`writers`: ఐచ్చిక అనుమతించబడిన రైటర్లు (list[Pubkey]) |
|----------------|--------------------------|
| **రిటర్న్స్** | లావాదేవీ సంతకం (str) |
...
| **పారామీటర్లు** | `connection`: AsyncClient<br/>`signer`: Keypair లేదా WalletSigner<br/>`db_root_id`: డేటాబేస్ ID (bytes లేదా str)<br/>`table_seed`: టేబుల్ పేరు (bytes లేదా str)<br/>`row_json`: JSON వరుస డేటా (str)<br/>`skip_confirmation`: tx నిర్ధారణను దాటవేయండి (డిఫాల్ట్: False)<br/>`remaining_accounts`: రీడ్-ఓన్లీ రిఫరెన్స్ ఖాతాలుగా జోడించడానికి Pubkey యొక్క ఐచ్ఛిక లిస్ట్ (ఉదా. ఫీడ్ PDA ఇండెక్సింగ్ కోసం) |
|----------------|--------------------------|
| **రిటర్న్స్** | లావాదేవీ సంతకం (str) |
...
| **పారామీటర్లు** | `sign_message`: వాలెట్ సైన్ ఫంక్షన్ (callable) |
|----------------|--------------------------|
| **రిటర్న్స్** | `priv_key` (bytes) మరియు `pub_key` (str) కలిగిన dict |
...
| **పారామీటర్లు** | `recipient_pub_hex`: గ్రహీత పబ్లిక్ కీ (హెక్స్ str)<br/>`plaintext`: ఎన్‌క్రిప్ట్ చేయవలసిన డేటా (bytes) |
|----------------|--------------------------|
| **రిటర్న్స్** | `sender_pub`, `iv`, `ciphertext` కలిగిన dict |

#### `dh_decrypt()`

| **పారామీటర్లు** | `recipient_priv_key`: గ్రహీత ప్రైవేట్ కీ (bytes)<br/>`sender_pub`: పంపినవారి పబ్లిక్ కీ (హెక్స్ str)<br/>`iv`: ఇనిషియలైజేషన్ వెక్టర్ (str)<br/>`ciphertext`: ఎన్‌క్రిప్టెడ్ టెక్స్ట్ (str) |
|----------------|--------------------------|
| **రిటర్న్స్** | bytes (ప్లెయిన్‌టెక్స్ట్) |
...
| **పారామీటర్లు** | `password`: పాస్‌వర్డ్ (str)<br/>`plaintext`: ఎన్‌క్రిప్ట్ చేయవలసిన డేటా (bytes) |
|----------------|--------------------------|
| **రిటర్న్స్** | `salt`, `iv`, `ciphertext` కలిగిన dict |

#### `password_decrypt()`

| **పారామీటర్లు** | `password`: పాస్‌వర్డ్ (str)<br/>`salt`: సాల్ట్ (str)<br/>`iv`: ఇనిషియలైజేషన్ వెక్టర్ (str)<br/>`ciphertext`: ఎన్‌క్రిప్టెడ్ టెక్స్ట్ (str) |
|----------------|--------------------------|
| **రిటర్న్స్** | bytes (ప్లెయిన్‌టెక్స్ట్) |
...
| **పారామీటర్లు** | `recipient_pub_hex_array`: గ్రహీతల పబ్లిక్ కీల జాబితా (list[str])<br/>`plaintext`: ఎన్‌క్రిప్ట్ చేయవలసిన డేటా (bytes) |
|----------------|--------------------------|
| **రిటర్న్స్** | బహుళ-గ్రహీత ఎన్‌క్రిప్టెడ్ ఆబ్జెక్ట్ |

#### `multi_decrypt()`

| **పారామీటర్లు** | `recipient_priv_key`: గ్రహీత ప్రైవేట్ కీ (bytes)<br/>`recipient_pub_hex`: గ్రహీత పబ్లిక్ కీ (హెక్స్ str)<br/>`encrypted`: బహుళ-గ్రహీత ఎన్‌క్రిప్టెడ్ ఆబ్జెక్ట్ |
|----------------|--------------------------|
| **రిటర్న్స్** | bytes (ప్లెయిన్‌టెక్స్ట్) |
...
| **మాడ్యూల్** | `writer` |
|------------|----------|
| **పారామీటర్లు** | `connection`: AsyncClient<br/>`signer`: Keypair లేదా WalletSigner<br/>`db_root_id`: డేటాబేస్ ID (bytes లేదా str)<br/>`seed`: టేబుల్ లేదా కనెక్షన్ సీడ్ (bytes లేదా str)<br/>`row_json`: JSON వరుస డేటా (str)<br/>`table_name`: టేబుల్ ఎడిట్‌లకు అవసరం (ఐచ్ఛికం)<br/>`target_tx`: టేబుల్ ఎడిట్‌ల కోసం సూచన tx (ఐచ్ఛికం) |
| **రిటర్న్స్** | లావాదేవీ సంతకం (str) |
