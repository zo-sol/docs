...
Use [`create_table()`](#create_table) para crear una tabla explícitamente. Si el PDA DbRoot tiene poco espacio, el SDK lo expande automáticamente en la misma transacción — sin pasos adicionales.
...
- [`create_table()`](#create_table): crear una nueva tabla (expande DbRoot automáticamente si es necesario)
- [`write_row()`](#write_row): agregar una nueva fila a una tabla existente
...
Se deriva un par de claves X25519 a partir de la firma del wallet mediante [`derive_x25519_keypair()`](#derive_x25519_keypair).

#### Funciones clave relacionadas

- [`derive_x25519_keypair()`](#derive_x25519_keypair): derivar par de claves de cifrado desde la firma del wallet
- [`dh_encrypt()`](#dh_encrypt) / [`dh_decrypt()`](#dh_decrypt): cifrado/descifrado para un solo destinatario
- [`password_encrypt()`](#password_encrypt) / [`password_decrypt()`](#password_decrypt): cifrado/descifrado basado en contraseña
- [`multi_encrypt()`](#multi_encrypt) / [`multi_decrypt()`](#multi_decrypt): cifrado/descifrado multi-destinatario
...
| **Parámetros** | `connection`: AsyncClient<br/>`signer`: Keypair o WalletSigner<br/>`db_root_id`: ID de base de datos (bytes o str)<br/>`connection_seed`: semilla de conexión (bytes o str)<br/>`row_json`: datos JSON (str) |
|----------------|--------------------------|
| **Retorna** | Firma de transacción (str) |
...
| **Parámetros** | `connection`: AsyncClient<br>`signer`: Keypair<br>`db_root_id`: ID de base de datos (bytes o str)<br>`meta`: metadatos a almacenar (bytes o str) |
|----------|--------------------------|
| **Devuelve** | Firma de transacción (str) |
...
| **Parámetros** | `connection`: AsyncClient<br/>`signer`: Keypair o WalletSigner<br/>`db_root_id`: ID de base de datos (bytes o str)<br/>`table_seed`: semilla de tabla (bytes o str)<br/>`table_name`: nombre de tabla (bytes o str)<br/>`column_names`: lista de columnas (list[bytes o str])<br/>`id_col`: columna ID (bytes o str)<br/>`ext_keys`: claves de extensión (list[bytes o str])<br/>`gate_mint`: gate de token opcional (Pubkey)<br/>`writers`: escritores permitidos opcionales (list[Pubkey]) |
|----------------|--------------------------|
| **Retorna** | Firma de transacción (str) |
...
| **Parámetros** | `connection`: AsyncClient<br/>`signer`: Keypair o WalletSigner<br/>`db_root_id`: ID de base de datos (bytes o str)<br/>`table_seed`: nombre de la tabla (bytes o str)<br/>`row_json`: datos JSON de la fila (str)<br/>`skip_confirmation`: omitir confirmación de tx (por defecto: False)<br/>`remaining_accounts`: lista opcional de Pubkey para agregar como cuentas de referencia de solo lectura (ej. feed PDA para indexación) |
|----------------|--------------------------|
| **Retorna** | Firma de transacción (str) |
...
| **Parámetros** | `sign_message`: función de firma del wallet (callable) |
|----------------|--------------------------|
| **Retorna** | dict con `priv_key` (bytes) y `pub_key` (str) |
...
| **Parámetros** | `recipient_pub_hex`: clave pública del destinatario (str hexadecimal)<br/>`plaintext`: datos a cifrar (bytes) |
|----------------|--------------------------|
| **Retorna** | dict con `sender_pub`, `iv`, `ciphertext` |

#### `dh_decrypt()`

| **Parámetros** | `recipient_priv_key`: clave privada del destinatario (bytes)<br/>`sender_pub`: clave pública del remitente (str hexadecimal)<br/>`iv`: vector de inicialización (str)<br/>`ciphertext`: texto cifrado (str) |
|----------------|--------------------------|
| **Retorna** | bytes (texto plano) |
...
| **Parámetros** | `password`: contraseña (str)<br/>`plaintext`: datos a cifrar (bytes) |
|----------------|--------------------------|
| **Retorna** | dict con `salt`, `iv`, `ciphertext` |

#### `password_decrypt()`

| **Parámetros** | `password`: contraseña (str)<br/>`salt`: sal (str)<br/>`iv`: vector de inicialización (str)<br/>`ciphertext`: texto cifrado (str) |
|----------------|--------------------------|
| **Retorna** | bytes (texto plano) |
...
| **Parámetros** | `recipient_pub_hex_array`: lista de claves públicas de destinatarios (list[str])<br/>`plaintext`: datos a cifrar (bytes) |
|----------------|--------------------------|
| **Retorna** | Objeto cifrado multi-destinatario |

#### `multi_decrypt()`

| **Parámetros** | `recipient_priv_key`: clave privada del destinatario (bytes)<br/>`recipient_pub_hex`: clave pública del destinatario (str hexadecimal)<br/>`encrypted`: objeto cifrado multi-destinatario |
|----------------|--------------------------|
| **Retorna** | bytes (texto plano) |
...
| **Módulo** | `writer` |
|------------|----------|
| **Parámetros** | `connection`: AsyncClient<br/>`signer`: Keypair o WalletSigner<br/>`db_root_id`: ID de base de datos (bytes o str)<br/>`seed`: semilla de tabla o conexión (bytes o str)<br/>`row_json`: datos JSON de la fila (str)<br/>`table_name`: requerido para ediciones de tabla (opcional)<br/>`target_tx`: tx de referencia para ediciones de tabla (opcional) |
| **Retorna** | Firma de transacción (str) |
