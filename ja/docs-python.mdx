...
[`create_table()`](#create_table) を使用してテーブルを明示的に作成します。DbRoot PDAの空き容量が不足している場合、SDKは同じトランザクション内で自動的に拡張します。追加の操作は不要です。
...
- [`create_table()`](#create_table): 新しいテーブルを作成（必要に応じてDbRootを自動拡張）
- [`write_row()`](#write_row)：既存テーブルに新しい行を追加
...
`derive_x25519_keypair` を使用してウォレット署名から X25519 キーペアを導出します。

#### 関連する主要関数

- [`derive_x25519_keypair()`](#derive_x25519_keypair)：ウォレット署名から X25519 キーペアを導出
- [`dh_encrypt()`](#dh_encrypt) / [`dh_decrypt()`](#dh_decrypt)：DH 暗号化/復号化
- [`password_encrypt()`](#password_encrypt) / [`password_decrypt()`](#password_decrypt)：パスワード暗号化/復号化
- [`multi_encrypt()`](#multi_encrypt) / [`multi_decrypt()`](#multi_decrypt)：マルチ受信者暗号化/復号化
...
| **パラメータ** | `connection`：AsyncClient<br/>`signer`：Keypair または WalletSigner<br/>`db_root_id`：データベースID（bytesまたはstr）<br/>`party_a`：1人目のユーザーの公開鍵（str）<br/>`party_b`：2人目のユーザーの公開鍵（str）<br/>`table_name`：コネクションテーブル名（strまたはbytes）<br/>`columns`：カラムリスト（list[strまたはbytes]）<br/>`id_col`：IDカラム（strまたはbytes）<br/>`ext_keys`：拡張キー（list[strまたはbytes]） |
|----------------|--------------------------|
| **戻り値** | トランザクション署名（str） |
...
| **パラメータ** | `connection`：AsyncClient<br/>`signer`：Keypair または WalletSigner<br/>`db_root_id`：データベースID（bytesまたはstr）<br/>`connection_seed`：コネクションシード（bytesまたはstr）<br/>`row_json`：JSONデータ（str） |
|----------------|--------------------------|
| **戻り値** | トランザクション署名（str） |
...
| **パラメータ** | `connection`: AsyncClient<br>`signer`: Keypair または WalletSigner<br>`db_root_id`: データベース ID（bytes または str）<br>`meta`: 保存するメタデータ（bytes または str） |
|----------|--------------------------|
| **戻り値** | トランザクション署名（str） |
...
| **パラメータ** | `connection`: AsyncClient<br/>`signer`: Keypair または WalletSigner<br/>`db_root_id`: データベースID (bytes または str)<br/>`table_seed`: テーブルシード (bytes または str)<br/>`table_name`: テーブル名 (bytes または str)<br/>`column_names`: カラムリスト (list[bytes または str])<br/>`id_col`: IDカラム (bytes または str)<br/>`ext_keys`: 拡張キー (list[bytes または str])<br/>`gate_mint`: オプションのトークンゲート (Pubkey)<br/>`writers`: オプションの許可ライター (list[Pubkey]) |
|----------------|--------------------------|
| **戻り値** | トランザクション署名 (str) |
...
| **パラメータ** | `connection`：AsyncClient<br/>`signer`：Keypair または WalletSigner<br/>`db_root_id`：データベースID（bytesまたはstr）<br/>`table_seed`：テーブル名（bytesまたはstr）<br/>`row_json`：JSON行データ（str）<br/>`skip_confirmation`：トランザクション確認をスキップ（デフォルト：False）<br/>`remaining_accounts`：読み取り専用の参照アカウントとして追加するPubkeyのリスト（オプション、例：フィードPDAインデックス用） |
|----------------|--------------------------|
| **戻り値** | トランザクション署名（str） |
...
| **パラメータ** | `sign_message`：メッセージ署名関数（async callable） |
|----------------|--------------------------|
| **戻り値** | `priv_key` と `pub_key` を含む辞書 |
...
| **パラメータ** | `recipient_pub_hex`：受信者の公開鍵（hex string）<br/>`plaintext`：暗号化するデータ |
|----------------|--------------------------|
| **戻り値** | `sender_pub`、`iv`、`ciphertext` を含む辞書 |
...
| **パラメータ** | `priv_key`：秘密鍵<br/>`sender_pub_hex`：送信者の公開鍵（hex string）<br/>`iv_hex`：初期化ベクトル（hex string）<br/>`ciphertext_hex`：暗号文（hex string） |
|----------------|--------------------------|
| **戻り値** | bytes |
...
| **パラメータ** | `password`：復号化パスワード<br/>`salt_hex`：ソルト（hex string）<br/>`iv_hex`：初期化ベクトル（hex string）<br/>`ciphertext_hex`：暗号文（hex string） |
|----------------|--------------------------|
| **戻り値** | bytes |
...
| **パラメータ** | `recipient_pub_hexes`：受信者の公開鍵リスト（list[str]）<br/>`plaintext`：暗号化するデータ |
|----------------|--------------------------|
| **戻り値** | `recipients`、`iv`、`ciphertext` を含む辞書 |
...
| **パラメータ** | `priv_key`：秘密鍵<br/>`pub_key_hex`：自分の公開鍵（hex string）<br/>`encrypted`：暗号化辞書 |
|----------------|--------------------------|
| **戻り値** | bytes |
...
| **モジュール** | `writer` |
|------------|----------|
| **パラメータ** | `connection`：AsyncClient<br/>`signer`：Keypair または WalletSigner<br/>`db_root_id`：データベースID（bytesまたはstr）<br/>`seed`：テーブルまたはコネクションシード（bytesまたはstr）<br/>`row_json`：JSON行データ（str）<br/>`table_name`：テーブル編集時に必要（オプション）<br/>`target_tx`：テーブル編集用の参照トランザクション（オプション） |
| **戻り値** | トランザクション署名（str） |
