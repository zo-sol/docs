...
| **パラメータ** | `connection`: AsyncClient<br>`signer`: Keypair<br>`db_root_id`: データベース ID（bytes または str）<br>`meta`: 保存するメタデータ（bytes または str） |
|----------|--------------------------|
| **戻り値** | トランザクション署名（str） |
...
| **パラメータ** | `connection`：AsyncClient<br/>`signer`：Keypair<br/>`db_root_id`：データベースID（bytesまたはstr）<br/>`table_seed`：テーブル名（bytesまたはstr）<br/>`row_json`：JSON行データ（str）<br/>`skip_confirmation`：トランザクション確認をスキップ（デフォルト：False）<br/>`remaining_accounts`：読み取り専用の参照アカウントとして追加するPubkeyのリスト（オプション、例：フィードPDAインデックス用） |
|----------------|--------------------------|
| **戻り値** | トランザクション署名（str） |
