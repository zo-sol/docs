...
| **Parameters** | `connection`: Solana RPC Connection<br/>`signer`: Signer<br/>`dbRootId`: database ID (Uint8Array or string)<br/>`tableSeed`: table seed (Uint8Array or string)<br/>`tableName`: table name (Uint8Array or string)<br/>`columnNames`: column list (Array\<Uint8Array \| string\>)<br/>`idCol`: ID column (Uint8Array or string)<br/>`extKeys`: extension keys (Array\<Uint8Array \| string\>)<br/>`gateMint`: optional token gate (PublicKey)<br/>`writers`: optional allowed writers (PublicKey[]) |
|----------------|--------------------------|
| **Returns** | Transaction signature (string) |
