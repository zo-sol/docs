...
<Note>
The gateway reads Solana version-1 transactions: its readers request
`maxSupportedTransactionVersion: 1`, which also returns legacy and v0
transactions. Files written through the SDK's
[v1 pipeline](/docs-typescript#transaction-v1-support) are served exactly like
legacy writes, from the same URLs.
</Note>
