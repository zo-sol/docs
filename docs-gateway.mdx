...
# {"status":"ok","mode":"multi","chains":["solana","sepolia","monad","monadTestnet"], ... }
```
...
| Variable | Required | Description |
|---|---|---|
| `IQETH_NETWORK` | Yes in `evm` mode | `sepolia`, `monad`, or `monadTestnet`. |
...
1. **`?network=` wins.** `solana`, `sepolia`, `monad`, or `monadTestnet`. An
...
# {"error":"unknown network \"foo\" (expected: solana | sepolia | monad | monadTestnet)"}
```
