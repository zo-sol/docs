...
[`create_table()`](#create_table)을 사용하여 테이블을 명시적으로 생성합니다. DbRoot PDA의 공간이 부족하면, SDK가 같은 트랜잭션 내에서 자동으로 확장합니다 — 추가 작업이 필요 없습니다.
...
- [`create_table()`](#create_table): 새 테이블 생성 (필요시 DbRoot 자동 확장)
- [`write_row()`](#write_row): 기존 테이블에 새 행 추가
...
[`derive_x25519_keypair()`](#derive_x25519_keypair)를 사용하여 지갑 서명으로부터 결정적 X25519 키쌍을 도출할 수 있습니다. 사용자가 별도의 암호화 키를 관리할 필요 없이 — 지갑이 곧 키입니다.

#### 관련 주요 함수

- [`derive_x25519_keypair()`](#derive_x25519_keypair): 지갑에서 암호화 키쌍 도출
- [`dh_encrypt()`](#dh_encrypt) / [`dh_decrypt()`](#dh_decrypt): 단일 수신자 암호화
- [`password_encrypt()`](#password_encrypt) / [`password_decrypt()`](#password_decrypt): 비밀번호 기반 암호화
- [`multi_encrypt()`](#multi_encrypt) / [`multi_decrypt()`](#multi_decrypt): 다중 수신자 암호화
...
| **파라미터** | `connection`: AsyncClient<br/>`signer`: Keypair 또는 WalletSigner<br/>`db_root_id`: 데이터베이스 ID (bytes 또는 str)<br/>`party_a`: 첫 번째 사용자 공개키 (str)<br/>`party_b`: 두 번째 사용자 공개키 (str)<br/>`table_name`: 연결 테이블 이름 (str 또는 bytes)<br/>`columns`: 컬럼 목록 (list[str 또는 bytes])<br/>`id_col`: ID 컬럼 (str 또는 bytes)<br/>`ext_keys`: 확장 키 (list[str 또는 bytes]) |
|----------------|--------------------------|
| **반환값** | 트랜잭션 서명 (str) |
...
| **파라미터** | `connection`: AsyncClient<br/>`signer`: Keypair 또는 WalletSigner<br/>`db_root_id`: 데이터베이스 ID (bytes 또는 str)<br/>`connection_seed`: 연결 시드 (bytes 또는 str)<br/>`row_json`: JSON 데이터 (str) |
|----------------|--------------------------|
| **반환값** | 트랜잭션 서명 (str) |
...
| **파라미터** | `connection`: AsyncClient<br>`signer`: Keypair<br>`db_root_id`: 데이터베이스 ID (bytes 또는 str)<br>`meta`: 저장할 메타데이터 (bytes 또는 str) |
|----------|--------------------------|
| **반환값** | 트랜잭션 서명 (str) |
...
| **매개변수** | `connection`: AsyncClient<br/>`signer`: Keypair 또는 WalletSigner<br/>`db_root_id`: 데이터베이스 ID (bytes 또는 str)<br/>`table_seed`: 테이블 시드 (bytes 또는 str)<br/>`table_name`: 테이블 이름 (bytes 또는 str)<br/>`column_names`: 컬럼 목록 (list[bytes 또는 str])<br/>`id_col`: ID 컬럼 (bytes 또는 str)<br/>`ext_keys`: 확장 키 (list[bytes 또는 str])<br/>`gate_mint`: 선택적 토큰 게이트 (Pubkey)<br/>`writers`: 선택적 허용 작성자 (list[Pubkey]) |
|----------------|--------------------------|
| **반환값** | 트랜잭션 서명 (str) |
...
| **파라미터** | `connection`: AsyncClient<br/>`signer`: Keypair 또는 WalletSigner<br/>`db_root_id`: 데이터베이스 ID (bytes 또는 str)<br/>`table_seed`: 테이블 이름 (bytes 또는 str)<br/>`row_json`: JSON 행 데이터 (str)<br/>`skip_confirmation`: 트랜잭션 확인 생략 (기본값: False)<br/>`remaining_accounts`: 읽기 전용 참조 계정으로 추가할 Pubkey 리스트 (선택사항, 예: 피드 PDA 인덱싱용) |
|----------------|--------------------------|
| **반환값** | 트랜잭션 서명 (str) |
...
| **파라미터** | `sign_message`: 비동기 서명 함수 `Callable[[bytes], Awaitable[bytes]]` |
|-------------|--------------------------|
| **반환값** | `priv_key` (bytes)와 `pub_key` (bytes)를 포함하는 dict |
...
| **파라미터** | `recipient_pub_hex`: 수신자의 X25519 공개키 (hex str)<br/>`plaintext`: 암호화할 데이터 (bytes) |
|-------------|--------------------------|
| **반환값** | `sender_pub`, `iv`, `ciphertext` (모두 hex str)를 포함하는 dict |

#### `dh_decrypt()`

| **파라미터** | `priv_key`: 수신자의 개인키 (bytes)<br/>`sender_pub_hex`: 발신자의 임시 공개키 (hex str)<br/>`iv_hex`: IV (hex str)<br/>`ciphertext_hex`: 암호문 (hex str) |
|-------------|--------------------------|
| **반환값** | bytes (복호화된 평문) |
...
# 단일 수신자에게 암호화
encrypted = crypto.dh_encrypt(recipient_pub_hex, b'비밀 메시지')
...
| **파라미터** | `password`: 비밀번호 (str)<br/>`plaintext`: 암호화할 데이터 (bytes) |
|-------------|--------------------------|
| **반환값** | `salt`, `iv`, `ciphertext` (모두 hex str)를 포함하는 dict |

#### `password_decrypt()`

| **파라미터** | `password`: 비밀번호 (str)<br/>`salt_hex`: salt (hex str)<br/>`iv_hex`: IV (hex str)<br/>`ciphertext_hex`: 암호문 (hex str) |
|-------------|--------------------------|
| **반환값** | bytes (복호화된 평문) |
...
# 비밀번호로 암호화
encrypted = crypto.password_encrypt('my-password', b'비밀 데이터')
...
| **파라미터** | `recipient_pub_hexes`: 수신자 공개키 목록 (list[str])<br/>`plaintext`: 암호화할 데이터 (bytes) |
|-------------|--------------------------|
| **반환값** | `recipients` (list[RecipientEntry]), `iv`, `ciphertext`를 포함하는 dict |

#### `multi_decrypt()`

| **파라미터** | `priv_key`: 본인의 개인키 (bytes)<br/>`pub_key_hex`: 본인의 공개키 (hex str)<br/>`encrypted`: MultiEncryptResult dict |
|-------------|--------------------------|
| **반환값** | bytes (복호화된 평문) |
...
| **모듈** | `writer` |
|----------|----------|
| **파라미터** | `connection`: AsyncClient<br/>`signer`: Keypair 또는 WalletSigner<br/>`db_root_id`: 데이터베이스 ID (bytes 또는 str)<br/>`seed`: 테이블 또는 연결 시드 (bytes 또는 str)<br/>`row_json`: JSON 행 데이터 (str)<br/>`table_name`: 테이블 수정 시 필요 (선택)<br/>`target_tx`: 테이블 수정 시 참조 트랜잭션 (선택) |
| **반환값** | 트랜잭션 서명 (str) |
