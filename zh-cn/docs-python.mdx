...
使用 [`create_table()`](#create_table) 显式创建表。如果 DbRoot PDA 空间不足，SDK 会在同一交易中自动扩展，无需额外操作。
...
- [`create_table()`](#create_table)：创建新表（如需自动扩展 DbRoot）
- [`write_row()`](#write_row)：向现有表添加新行
...
SDK 提供 3 种加密模式以及从钱包签名派生密钥的功能。

#### 加密模式

- **DH（Diffie-Hellman）加密**：临时 X25519 ECDH → HKDF-SHA256 → AES-256-GCM。用于单个接收者的加密。
- **密码加密**：PBKDF2-SHA256（250,000 次迭代）→ AES-256-GCM。基于密码的加密。
- **多接收者加密**：PGP 风格的混合方案。生成随机 CEK（内容加密密钥），对每个接收者进行 ECDH 包装。

#### 密钥派生

使用 `derive_x25519_keypair` 从钱包签名派生 X25519 密钥对。

#### 关键相关函数

- [`derive_x25519_keypair()`](#derive_x25519_keypair)：从钱包签名派生 X25519 密钥对
- [`dh_encrypt()`](#dh_encrypt) / [`dh_decrypt()`](#dh_decrypt)：DH 加密/解密
- [`password_encrypt()`](#password_encrypt) / [`password_decrypt()`](#password_decrypt)：密码加密/解密
- [`multi_encrypt()`](#multi_encrypt) / [`multi_decrypt()`](#multi_decrypt)：多接收者加密/解密
...
| **参数** | `connection`：AsyncClient<br/>`signer`：Keypair 或 WalletSigner<br/>`db_root_id`：数据库 ID（bytes 或 str）<br/>`party_a`：第一个用户公钥（str）<br/>`party_b`：第二个用户公钥（str）<br/>`table_name`：连接表名（str 或 bytes）<br/>`columns`：列列表（list[str 或 bytes]）<br/>`id_col`：ID 列（str 或 bytes）<br/>`ext_keys`：扩展键（list[str 或 bytes]） |
|----------------|--------------------------|
| **返回值** | 交易签名（str） |
...
| **参数** | `connection`：AsyncClient<br/>`signer`：Keypair 或 WalletSigner<br/>`db_root_id`：数据库 ID（bytes 或 str）<br/>`connection_seed`：连接种子（bytes 或 str）<br/>`row_json`：JSON 数据（str） |
|----------------|--------------------------|
| **返回值** | 交易签名（str） |
...
| **参数** | `connection`：AsyncClient<br>`signer`：Keypair 或 WalletSigner<br>`db_root_id`：数据库 ID（bytes 或 str）<br>`meta`：要存储的元数据（bytes 或 str） |
|----------|--------------------------|
| **返回** | 交易签名（str） |
...
| **参数** | `connection`: AsyncClient<br/>`signer`: Keypair 或 WalletSigner<br/>`db_root_id`: 数据库 ID (bytes 或 str)<br/>`table_seed`: 表种子 (bytes 或 str)<br/>`table_name`: 表名 (bytes 或 str)<br/>`column_names`: 列名列表 (list[bytes 或 str])<br/>`id_col`: ID 列 (bytes 或 str)<br/>`ext_keys`: 扩展键 (list[bytes 或 str])<br/>`gate_mint`: 可选的代币门控 (Pubkey)<br/>`writers`: 可选的授权写入者 (list[Pubkey]) |
|----------------|--------------------------|
| **返回值** | 交易签名 (str) |

如果 DbRoot PDA 空间不足，realloc 指令会自动添加到同一交易中。无需额外签名或手动空间管理。
...
| **参数** | `connection`：AsyncClient<br/>`signer`：Keypair 或 WalletSigner<br/>`db_root_id`：数据库 ID（bytes 或 str）<br/>`table_seed`：表名（bytes 或 str）<br/>`row_json`：JSON 行数据（str）<br/>`skip_confirmation`：跳过交易确认（默认值：False）<br/>`remaining_accounts`：作为只读引用账户追加的 Pubkey 列表（可选，例如用于 feed PDA 索引） |
|----------------|--------------------------|
| **返回值** | 交易签名（str） |
...
| **参数** | `sign_message`：消息签名函数（async callable） |
|----------------|--------------------------|
| **返回值** | 包含 `priv_key` 和 `pub_key` 的字典 |
...
| **参数** | `recipient_pub_hex`：接收者公钥（hex string）<br/>`plaintext`：要加密的数据 |
|----------------|--------------------------|
| **返回值** | 包含 `sender_pub`、`iv`、`ciphertext` 的字典 |
...
| **参数** | `priv_key`：私钥<br/>`sender_pub_hex`：发送者公钥（hex string）<br/>`iv_hex`：初始化向量（hex string）<br/>`ciphertext_hex`：密文（hex string） |
|----------------|--------------------------|
| **返回值** | bytes |
...
| **参数** | `password`：加密密码<br/>`plaintext`：要加密的数据 |
|----------------|--------------------------|
| **返回值** | 包含 `salt`、`iv`、`ciphertext` 的字典 |
...
| **参数** | `password`：解密密码<br/>`salt_hex`：盐值（hex string）<br/>`iv_hex`：初始化向量（hex string）<br/>`ciphertext_hex`：密文（hex string） |
|----------------|--------------------------|
| **返回值** | bytes |
...
| **参数** | `recipient_pub_hexes`：接收者公钥列表（list[str]）<br/>`plaintext`：要加密的数据 |
|----------------|--------------------------|
| **返回值** | 包含 `recipients`、`iv`、`ciphertext` 的字典 |
...
| **参数** | `priv_key`：私钥<br/>`pub_key_hex`：自己的公钥（hex string）<br/>`encrypted`：加密字典 |
|----------------|--------------------------|
| **返回值** | bytes |
...
| **模块** | `writer` |
|------------|----------|
| **参数** | `connection`：AsyncClient<br/>`signer`：Keypair 或 WalletSigner<br/>`db_root_id`：数据库 ID（bytes 或 str）<br/>`seed`：表或连接种子（bytes 或 str）<br/>`row_json`：JSON 行数据（str）<br/>`table_name`：表编辑时必需（可选）<br/>`target_tx`：表编辑的参考交易（可选） |
| **返回值** | 交易签名（str） |
| **使用场景** | 自定义行管理、更新现有行 |
