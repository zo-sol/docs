...
使用 [`createTable()`](#createtable) 显式创建表。如果 DbRoot PDA 空间不足，SDK 会在同一交易中自动扩展，无需额外操作。
...
- [`createTable()`](#createtable)：创建新表（如需自动扩展 DbRoot）
- [`writeRow()`](#writerow)：向现有表添加新行
...
使用 `deriveX25519Keypair` 从钱包签名派生 X25519 密钥对。

#### 关键相关函数

- [`deriveX25519Keypair()`](#derivex25519keypair)：从钱包签名派生 X25519 密钥对
- [`dhEncrypt()`](#dhencrypt) / [`dhDecrypt()`](#dhdecrypt)：DH 加密/解密
- [`passwordEncrypt()`](#passwordencrypt) / [`passwordDecrypt()`](#passworddecrypt)：密码加密/解密
- [`multiEncrypt()`](#multiencrypt) / [`multiDecrypt()`](#multidecrypt)：多接收者加密/解密
...
| **参数** | `connection`：Solana RPC 连接<br/>`signer`：签名者<br/>`dbRootId`：数据库 ID（Uint8Array 或 string）<br/>`meta`：要存储的元数据（Uint8Array 或 string） |
|----------------|--------------------------|
| **返回** | 交易签名（string） |
...
| **参数** | `connection`: Solana RPC Connection<br/>`signer`: Signer<br/>`dbRootId`: 数据库 ID (Uint8Array 或 string)<br/>`tableSeed`: 表种子 (Uint8Array 或 string)<br/>`tableName`: 表名 (Uint8Array 或 string)<br/>`columnNames`: 列名列表 (Array\<Uint8Array \| string\>)<br/>`idCol`: ID 列 (Uint8Array 或 string)<br/>`extKeys`: 扩展键 (Array\<Uint8Array \| string\>)<br/>`gateMint`: 可选的代币门控 (PublicKey)<br/>`writers`: 可选的授权写入者 (PublicKey[]) |
|----------------|--------------------------|
| **返回值** | 交易签名 (string) |
...
| **参数** | `connection`：Solana RPC 连接<br/>`signer`：签名者<br/>`dbRootId`：数据库 ID（Uint8Array 或 string）<br/>`tableSeed`：表名（Uint8Array 或 string）<br/>`rowJson`：JSON 行数据（string）<br/>`skipConfirmation`：跳过交易确认（boolean，默认：false）<br/>`remainingAccounts`：作为只读引用账户追加的 PublicKey 数组（可选，例如用于 feed PDA 索引） |
|----------------|--------------------------|
| **返回** | 交易签名（string） |
...
console.log(keypair.pubKey); // 公钥（hex）
```
...
| **参数** | `recipientPubHex`：接收者公钥（hex string）<br/>`plaintext`：要加密的数据 |
|----------------|--------------------------|
| **返回** | `{ senderPub, iv, ciphertext }` |
...
| **参数** | `privKey`：私钥<br/>`senderPubHex`：发送者公钥（hex string）<br/>`ivHex`：初始化向量（hex string）<br/>`ciphertextHex`：密文（hex string） |
|----------------|--------------------------|
| **返回** | Uint8Array |
...
| **参数** | `password`：解密密码<br/>`saltHex`：盐值（hex string）<br/>`ivHex`：初始化向量（hex string）<br/>`ciphertextHex`：密文（hex string） |
|----------------|--------------------------|
| **返回** | Uint8Array |
...
| **参数** | `recipientPubHexes`：接收者公钥数组（hex string[]）<br/>`plaintext`：要加密的数据 |
|----------------|--------------------------|
| **返回** | `{ recipients, iv, ciphertext }` |
...
| **参数** | `privKey`：私钥<br/>`pubKeyHex`：自己的公钥（hex string）<br/>`encrypted`：加密对象 |
|----------------|--------------------------|
| **返回** | Uint8Array |
