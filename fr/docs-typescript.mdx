...
Utilisez [`createTable()`](#createtable) pour créer une table explicitement. Si le PDA DbRoot manque d'espace, le SDK l'étend automatiquement dans la même transaction — aucune étape supplémentaire requise.
...
- [`createTable()`](#createtable) : créer une nouvelle table (étend DbRoot automatiquement si nécessaire)
- [`writeRow()`](#writerow) : ajouter une nouvelle ligne à une table existante
...
Une paire de clés X25519 est dérivée à partir de la signature du portefeuille via [`deriveX25519Keypair()`](#derivex25519keypair).

#### Fonctions clés associées

- [`deriveX25519Keypair()`](#derivex25519keypair) : dériver la paire de clés de chiffrement depuis la signature du portefeuille
- [`dhEncrypt()`](#dhencrypt) / [`dhDecrypt()`](#dhdecrypt) : chiffrement/déchiffrement pour un seul destinataire
- [`passwordEncrypt()`](#passwordencrypt) / [`passwordDecrypt()`](#passworddecrypt) : chiffrement/déchiffrement par mot de passe
- [`multiEncrypt()`](#multiencrypt) / [`multiDecrypt()`](#multidecrypt) : chiffrement/déchiffrement multi-destinataires
...
| **Paramètres** | `connection`: connexion RPC Solana<br/>`signer`: Signer<br/>`dbRootId`: ID de base de données (Uint8Array ou string)<br/>`meta`: métadonnées à stocker (Uint8Array ou string) |
|----------------|--------------------------|
| **Retourne** | Signature de transaction (string) |
...
| **Paramètres** | `connection` : Solana RPC Connection<br/>`signer` : Signer<br/>`dbRootId` : ID de base de données (Uint8Array ou string)<br/>`tableSeed` : seed de table (Uint8Array ou string)<br/>`tableName` : nom de table (Uint8Array ou string)<br/>`columnNames` : liste de colonnes (Array\<Uint8Array \| string\>)<br/>`idCol` : colonne ID (Uint8Array ou string)<br/>`extKeys` : clés d'extension (Array\<Uint8Array \| string\>)<br/>`gateMint` : gate de token optionnel (PublicKey)<br/>`writers` : écrivains autorisés optionnels (PublicKey[]) |
|----------------|--------------------------|
| **Retourne** | Signature de transaction (string) |
...
| **Paramètres** | `connection`: connexion RPC Solana<br/>`signer`: Signer<br/>`dbRootId`: ID de base de données (Uint8Array ou string)<br/>`tableSeed`: nom de la table (Uint8Array ou string)<br/>`rowJson`: données JSON de la ligne (string)<br/>`skipConfirmation`: ignorer la confirmation tx (boolean, par défaut : false)<br/>`remainingAccounts`: tableau optionnel de PublicKey à ajouter comme comptes de référence en lecture seule (ex. feed PDA pour l'indexation) |
|----------------|--------------------------|
| **Retourne** | Signature de transaction (string) |
...
| **Paramètres** | `signMessage` : fonction de signature du portefeuille `(message: Uint8Array) => Promise<Uint8Array>` |
|----------------|--------------------------|
| **Retourne** | `{ privKey: Uint8Array, pubKey: string }` |
...
| **Paramètres** | `recipientPubHex` : clé publique du destinataire (string hexadécimale)<br/>`plaintext` : données à chiffrer (Uint8Array) |
|----------------|--------------------------|
| **Retourne** | `{ senderPub: string, iv: string, ciphertext: string }` |

#### `dhDecrypt()`

| **Paramètres** | `recipientPrivKey` : clé privée du destinataire (Uint8Array)<br/>`senderPub` : clé publique de l'expéditeur (string hexadécimale)<br/>`iv` : vecteur d'initialisation (string)<br/>`ciphertext` : texte chiffré (string) |
|----------------|--------------------------|
| **Retourne** | `Uint8Array` (texte clair) |
...
| **Paramètres** | `password` : mot de passe (string)<br/>`plaintext` : données à chiffrer (Uint8Array) |
|----------------|--------------------------|
| **Retourne** | `{ salt: string, iv: string, ciphertext: string }` |

#### `passwordDecrypt()`

| **Paramètres** | `password` : mot de passe (string)<br/>`salt` : sel (string)<br/>`iv` : vecteur d'initialisation (string)<br/>`ciphertext` : texte chiffré (string) |
|----------------|--------------------------|
| **Retourne** | `Uint8Array` (texte clair) |
...
| **Paramètres** | `recipientPubHexArray` : tableau de clés publiques des destinataires (string[])<br/>`plaintext` : données à chiffrer (Uint8Array) |
|----------------|--------------------------|
| **Retourne** | Objet chiffré multi-destinataires |

#### `multiDecrypt()`

| **Paramètres** | `recipientPrivKey` : clé privée du destinataire (Uint8Array)<br/>`recipientPubHex` : clé publique du destinataire (string hexadécimale)<br/>`encrypted` : objet chiffré multi-destinataires |
|----------------|--------------------------|
| **Retourne** | `Uint8Array` (texte clair) |
