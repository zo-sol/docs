...
**Inscription cost (network fees).** Every chunk transaction pays the 5,000-lamport base fee, and the chunk size depends on the transaction profile the SDK selects:

- **Legacy profile** (wallet signers): 850-byte chunks, roughly **0.006 SOL per MB** of payload.
- **v1 profile** (keypair signers, with the transaction v1 feature gate active): 3,600-byte chunks, about 292 transactions per MB, roughly **0.0015 SOL per MB** of payload.

Binary data is base64-encoded (+33%), so budget about **0.008 SOL** (legacy) or **0.002 SOL** (v1) per MB of raw image, plus priority fees when the network is busy. A batch inscription script signing with a `Keypair` gets the v1 profile automatically, cutting network fees by about 4x.
...
|---|---|
| 777 images (~117 MB raw, base64 ~155 MB), network fees | ~1 SOL (legacy) or ~0.25 SOL (v1) |
| 777 metadata JSONs, network fees | ~0.01 SOL |
| Protocol fee, 1,554 uploads | ~2 to 8 SOL (unchanged by the transaction format) |
