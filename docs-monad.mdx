...
<Warning>
**Monad mainnet** is live and supported. Contract is deployed and ready. Switch with [`setNetwork('monad')`](#setnetwork).
</Warning>
...
The SDK ships with two network modes. Set Monad once at app startup with [`setNetwork()`](#setnetwork).
...
iqlabs.setNetwork('monad'); // call once at app startup
```
...
Store any data (files, text, JSON) directly on-chain. Data lives in transaction calldata — nothing goes to a centralized server. Reads reconstruct data by walking a linked list of transactions.
...
No explicit "create user" step. The first [`codeIn()`](#codein) call writes both the inventory entry and advances the chain tail. Each tx-chain write costs a small `BASIC_FEE` (0.0001 MON).
...
The connection seed is derived deterministically from the two addresses via [`deriveDmSeed(userA, userB)`](#derivedmseed) — either party can recompute it.
...
2. Call [`createTable()`](#createtable) to create a table. `LINKED_LIST_FEE` (0.0003 MON) is charged here.
...
Built-in encryption module (`iqlabs.crypto`) for encrypting data before storing on-chain. Primitives are identical to the Solana SDK — same plaintext flows across chains.
...
|----------|-------|--------------|
| `BASIC_FEE` | 0.0001 MON | Every `codeIn()` |
| `LINKED_LIST_FEE` | 0.0003 MON | `createTable`, `requestConnection`, `writeRow`, `writeConnectionRow` |

Monad is fast and cheap — these fees are tiny.
...
`LINKED_LIST_FEE` (0.0003 MON) charged here.
...
Create a new table. Charges `LINKED_LIST_FEE` (0.0003 MON).
...
Append a row to an existing table. Charges `LINKED_LIST_FEE` (0.0003 MON).
...
| **Parameters** | `mode`: `'sepolia' \| 'monad'`<br/>`rpcUrl`: optional override (string) |
|----------------|--------------------------|
| **Returns** | void |
...
// Submit a fortune (costs 0.0003 MON)
