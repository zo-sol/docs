...
<Warning>
**Sepolia testnet only.** The contract is not deployed to Ethereum mainnet yet. Do not use real-value assets with this SDK. All examples and the default RPC target Sepolia.
</Warning>
...
The default (and currently only) deployment lives on **Sepolia testnet** at:

```
0xa580a977a3103565993531d19536220A54783397
```

Mainnet deployment is not yet available.

The reader RPC defaults to `https://rpc.sepolia.org`. Override it with [`setRpcUrl()`](#setrpcurl) or via the env vars `IQLABS_RPC_ENDPOINT`, `ETHEREUM_RPC_URL`, or `RPC_URL`.
...
Override the RPC URL used by all reader functions. Default: `https://rpc.sepolia.org`.
...
<Note>
Writers (`signer`-based functions) use the provider attached to the `Signer` they receive. `setRpcUrl` does **not** affect them. The setting only routes reader RPC calls.
</Note>

If `setRpcUrl` is never called, the SDK falls back to env vars in this order: `IQLABS_RPC_ENDPOINT`, `ETHEREUM_RPC_URL`, `RPC_URL`, then the default.

#### `getRpcUrl()`

Returns the currently active RPC URL (resolving the same fallback chain).
