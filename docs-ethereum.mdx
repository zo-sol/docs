...
<Warning>
**Sepolia testnet** (default), **Monad mainnet**, and **Robinhood Chain mainnet** are supported. Ethereum mainnet is not deployed yet. Switch networks with [`setNetwork()`](#setnetwork). See [Network](#network).
</Warning>
...
The SDK ships with multiple network modes. Default is `sepolia`. Switch with [`setNetwork()`](#setnetwork) typically once at app startup.
...
iqlabs.setNetwork('monad'); // or 'robinhood', or 'sepolia' (default)
```

See the [Robinhood Chain guide](/docs-robinhood) for chain-specific setup (`robinhood` requires SDK >= 0.3.0).
...
| **Parameters** | `mode`: `'sepolia' \| 'monad' \| 'monadTestnet' \| 'robinhood'`<br />`rpcUrl`: optional override (string). Omitted → use the mode's default RPC. |
|----------------|--------------------------|
| **Returns** | void |
