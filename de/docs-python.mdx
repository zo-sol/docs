...
Verwenden Sie [`create_table()`](#create_table), um eine Tabelle explizit zu erstellen. Wenn der DbRoot-PDA wenig Platz hat, erweitert das SDK ihn automatisch in derselben Transaktion — keine zusätzlichen Schritte erforderlich.
...
- [`create_table()`](#create_table): Neue Tabelle erstellen (erweitert DbRoot bei Bedarf automatisch)
- [`write_row()`](#write_row): Neue Zeile zu einer bestehenden Tabelle hinzufügen
...
Verwenden Sie `derive_x25519_keypair`, um ein X25519-Schlüsselpaar aus einer Wallet-Signatur abzuleiten.

#### Wichtige zugehörige Funktionen

- [`derive_x25519_keypair()`](#derive_x25519_keypair): X25519-Schlüsselpaar aus Wallet-Signatur ableiten
- [`dh_encrypt()`](#dh_encrypt) / [`dh_decrypt()`](#dh_decrypt): DH-Verschlüsselung/-Entschlüsselung
- [`password_encrypt()`](#password_encrypt) / [`password_decrypt()`](#password_decrypt): Passwort-Verschlüsselung/-Entschlüsselung
- [`multi_encrypt()`](#multi_encrypt) / [`multi_decrypt()`](#multi_decrypt): Multi-Empfänger-Verschlüsselung/-Entschlüsselung
...
| **Parameter** | `connection`: AsyncClient<br/>`signer`: Keypair oder WalletSigner<br/>`db_root_id`: Datenbank-ID (bytes oder str)<br/>`party_a`: Public Key des ersten Benutzers (str)<br/>`party_b`: Public Key des zweiten Benutzers (str)<br/>`table_name`: Name der Verbindungstabelle (str oder bytes)<br/>`columns`: Spaltenliste (list[str oder bytes])<br/>`id_col`: ID-Spalte (str oder bytes)<br/>`ext_keys`: Erweiterungsschlüssel (list[str oder bytes]) |
|----------------|--------------------------|
| **Rückgabe** | Transaktionssignatur (str) |
...
| **Parameter** | `connection`: AsyncClient<br/>`signer`: Keypair oder WalletSigner<br/>`db_root_id`: Datenbank-ID (bytes oder str)<br/>`connection_seed`: Verbindungs-Seed (bytes oder str)<br/>`row_json`: JSON-Daten (str) |
|----------------|--------------------------|
| **Rückgabe** | Transaktionssignatur (str) |
...
| **Parameter** | `connection`: AsyncClient<br>`signer`: Keypair oder WalletSigner<br>`db_root_id`: Datenbank-ID (bytes oder str)<br>`meta`: zu speichernde Metadaten (bytes oder str) |
|----------|--------------------------|
| **Rückgabe** | Transaktionssignatur (str) |
...
# Profil-Metadaten auf der Blockchain speichern
...
| **Parameter** | `connection`: AsyncClient<br/>`signer`: Keypair oder WalletSigner<br/>`db_root_id`: Datenbank-ID (bytes oder str)<br/>`table_seed`: Tabellen-Seed (bytes oder str)<br/>`table_name`: Tabellenname (bytes oder str)<br/>`column_names`: Spaltenliste (list[bytes oder str])<br/>`id_col`: ID-Spalte (bytes oder str)<br/>`ext_keys`: Erweiterungsschlüssel (list[bytes oder str])<br/>`gate_mint`: Optionales Token-Gate (Pubkey)<br/>`writers`: Optionale erlaubte Schreiber (list[Pubkey]) |
|----------------|--------------------------|
| **Rückgabe** | Transaktionssignatur (str) |
...
| **Parameter** | `connection`: AsyncClient<br/>`signer`: Keypair oder WalletSigner<br/>`db_root_id`: Datenbank-ID (bytes oder str)<br/>`table_seed`: Tabellenname (bytes oder str)<br/>`row_json`: JSON-Zeilendaten (str)<br/>`skip_confirmation`: Transaktionsbestätigung überspringen (Standard: False)<br/>`remaining_accounts`: optionale Liste von Pubkey als schreibgeschützte Referenzkonten (z.B. Feed-PDA für Indexierung) |
|----------------|--------------------------|
| **Rückgabe** | Transaktionssignatur (str) |
...
| **Parameter** | `sign_message`: Nachrichtensignaturfunktion (async callable) |
|----------------|--------------------------|
| **Rückgabe** | dict mit `priv_key` und `pub_key` |

```python
from iqlabs import crypto

# X25519-Schlüsselpaar aus Wallet-Signatur ableiten
keypair = await crypto.derive_x25519_keypair(sign_message)
print(keypair['pub_key'])  # Öffentlicher Schlüssel (hex)
```
...
| **Parameter** | `recipient_pub_hex`: öffentlicher Schlüssel des Empfängers (hex string)<br/>`plaintext`: zu verschlüsselnde Daten |
|----------------|--------------------------|
| **Rückgabe** | dict mit `sender_pub`, `iv`, `ciphertext` |

```python
from iqlabs import crypto

# Daten mit DH-Verschlüsselung für den Empfänger verschlüsseln
encrypted = await crypto.dh_encrypt(recipient_pub_hex, plaintext)
```
...
| **Parameter** | `priv_key`: privater Schlüssel<br/>`sender_pub_hex`: öffentlicher Schlüssel des Senders (hex string)<br/>`iv_hex`: Initialisierungsvektor (hex string)<br/>`ciphertext_hex`: Chiffretext (hex string) |
|----------------|--------------------------|
| **Rückgabe** | bytes |
...
decrypted = crypto.dh_decrypt(priv_key, sender_pub_hex, iv_hex, ciphertext_hex)
```
...
|----------------|--------------------------|
| **Rückgabe** | dict mit `salt`, `iv`, `ciphertext` |

```python
from iqlabs import crypto

# Daten mit Passwort verschlüsseln
encrypted = crypto.password_encrypt(password, plaintext)
```
...
| **Parameter** | `password`: Entschlüsselungspasswort<br/>`salt_hex`: Salt (hex string)<br/>`iv_hex`: Initialisierungsvektor (hex string)<br/>`ciphertext_hex`: Chiffretext (hex string) |
|----------------|--------------------------|
| **Rückgabe** | bytes |
...
decrypted = crypto.password_decrypt(password, salt_hex, iv_hex, ciphertext_hex)
```
...
| **Parameter** | `recipient_pub_hexes`: Liste öffentlicher Empfängerschlüssel (list[str])<br/>`plaintext`: zu verschlüsselnde Daten |
|----------------|--------------------------|
| **Rückgabe** | dict mit `recipients`, `iv`, `ciphertext` |

```python
from iqlabs import crypto

# Daten für mehrere Empfänger verschlüsseln
encrypted = crypto.multi_encrypt(recipient_pub_hexes, plaintext)
```
...
| **Parameter** | `priv_key`: privater Schlüssel<br/>`pub_key_hex`: eigener öffentlicher Schlüssel (hex string)<br/>`encrypted`: Verschlüsselungs-dict |
|----------------|--------------------------|
| **Rückgabe** | bytes |
...
decrypted = crypto.multi_decrypt(priv_key, pub_key_hex, encrypted)
```
...
| **Modul** | `writer` |
|------------|----------|
| **Parameter** | `connection`: AsyncClient<br/>`signer`: Keypair oder WalletSigner<br/>`db_root_id`: Datenbank-ID (bytes oder str)<br/>`seed`: Tabellen- oder Verbindungs-Seed (bytes oder str)<br/>`row_json`: JSON-Zeilendaten (str)<br/>`table_name`: erforderlich für Tabellenbearbeitungen (optional)<br/>`target_tx`: Referenz-Transaktion für Tabellenbearbeitungen (optional) |
| **Rückgabe** | Transaktionssignatur (str) |
