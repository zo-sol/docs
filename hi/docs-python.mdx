...
टेबल को स्पष्ट रूप से बनाने के लिए [`create_table()`](#create_table) का उपयोग करें। यदि DbRoot PDA में जगह कम है, तो SDK उसी ट्रांज़ैक्शन में स्वचालित रूप से इसे विस्तारित करता है — कोई अतिरिक्त कदम आवश्यक नहीं।
...
- [`create_table()`](#create_table): नई टेबल बनाएं (आवश्यकता होने पर DbRoot स्वचालित रूप से विस्तारित)
- [`write_row()`](#write_row): मौजूदा टेबल में नई पंक्ति जोड़ें
...
SDK ऑन-चेन डेटा को सुरक्षित करने के लिए एंड-टू-एंड क्रिप्टोग्राफ़िक फ़ंक्शन प्रदान करता है।

#### एन्क्रिप्शन मोड

- **DH एन्क्रिप्शन (एकल प्राप्तकर्ता)**: एफ़ेमरल X25519 ECDH → HKDF-SHA256 → AES-256-GCM
- **पासवर्ड एन्क्रिप्शन**: PBKDF2-SHA256 (250,000 इटरेशन) → AES-256-GCM
- **मल्टी-प्राप्तकर्ता एन्क्रिप्शन (PGP-स्टाइल हाइब्रिड)**: रैंडम CEK, एक बार एन्क्रिप्ट, प्रत्येक प्राप्तकर्ता के लिए ECDH द्वारा CEK रैप

#### की डेरिवेशन

वॉलेट सिग्नेचर से [`derive_x25519_keypair()`](#derive_x25519_keypair) के माध्यम से X25519 की-पेयर डेराइव किया जाता है।

#### मुख्य संबंधित फ़ंक्शन

- [`derive_x25519_keypair()`](#derive_x25519_keypair): वॉलेट सिग्नेचर से एन्क्रिप्शन की-पेयर डेराइव करें
- [`dh_encrypt()`](#dh_encrypt) / [`dh_decrypt()`](#dh_decrypt): एकल प्राप्तकर्ता एन्क्रिप्शन/डिक्रिप्शन
- [`password_encrypt()`](#password_encrypt) / [`password_decrypt()`](#password_decrypt): पासवर्ड-आधारित एन्क्रिप्शन/डिक्रिप्शन
- [`multi_encrypt()`](#multi_encrypt) / [`multi_decrypt()`](#multi_decrypt): मल्टी-प्राप्तकर्ता एन्क्रिप्शन/डिक्रिप्शन
...
| **पैरामीटर** | `connection`: AsyncClient<br/>`signer`: Keypair या WalletSigner<br/>`db_root_id`: डेटाबेस ID (bytes या str)<br/>`party_a`: पहले यूज़र की pubkey (str)<br/>`party_b`: दूसरे यूज़र की pubkey (str)<br/>`table_name`: कनेक्शन टेबल नाम (str या bytes)<br/>`columns`: कॉलम सूची (list[str या bytes])<br/>`id_col`: ID कॉलम (str या bytes)<br/>`ext_keys`: एक्सटेंशन कीज़ (list[str या bytes]) |
|----------------|--------------------------|
| **रिटर्न** | ट्रांज़ैक्शन सिग्नेचर (str) |
...
| **पैरामीटर** | `connection`: AsyncClient<br/>`signer`: Keypair या WalletSigner<br/>`db_root_id`: डेटाबेस ID (bytes या str)<br/>`connection_seed`: कनेक्शन सीड (bytes या str)<br/>`row_json`: JSON डेटा (str) |
|----------------|--------------------------|
| **रिटर्न** | ट्रांज़ैक्शन सिग्नेचर (str) |
...
| **पैरामीटर्स** | `connection`: AsyncClient<br>`signer`: Keypair<br>`db_root_id`: डेटाबेस ID (bytes या str)<br>`meta`: स्टोर करने के लिए मेटाडेटा (bytes या str) |
|----------|--------------------------|
| **रिटर्न** | ट्रांजैक्शन सिग्नेचर (str) |

**उदाहरण:**
```python
from iqlabs import writer

# ब्लॉकचेन पर प्रोफ़ाइल मेटाडेटा स्टोर करें
...
| **पैरामीटर** | `connection`: AsyncClient<br/>`signer`: Keypair या WalletSigner<br/>`db_root_id`: डेटाबेस ID (bytes या str)<br/>`table_seed`: टेबल सीड (bytes या str)<br/>`table_name`: टेबल नाम (bytes या str)<br/>`column_names`: कॉलम सूची (list[bytes या str])<br/>`id_col`: ID कॉलम (bytes या str)<br/>`ext_keys`: एक्सटेंशन कीज़ (list[bytes या str])<br/>`gate_mint`: वैकल्पिक टोकन गेट (Pubkey)<br/>`writers`: वैकल्पिक अनुमत लेखक (list[Pubkey]) |
|----------------|--------------------------|
| **रिटर्न** | ट्रांज़ैक्शन सिग्नेचर (str) |

यदि DbRoot PDA में जगह कम है, तो उसी ट्रांज़ैक्शन में realloc इंस्ट्रक्शन स्वचालित रूप से जोड़ दिया जाता है। कोई अतिरिक्त हस्ताक्षर या मैन्युअल स्पेस प्रबंधन आवश्यक नहीं।
...
| **पैरामीटर** | `connection`: AsyncClient<br/>`signer`: Keypair या WalletSigner<br/>`db_root_id`: डेटाबेस ID (bytes या str)<br/>`table_seed`: टेबल नाम (bytes या str)<br/>`row_json`: JSON पंक्ति डेटा (str)<br/>`skip_confirmation`: tx कन्फ़र्मेशन छोड़ें (डिफ़ॉल्ट: False)<br/>`remaining_accounts`: रीड-ओनली रेफरेंस अकाउंट के रूप में जोड़ने के लिए Pubkey की वैकल्पिक लिस्ट (जैसे फीड PDA इंडेक्सिंग के लिए) |
|----------------|--------------------------|
| **रिटर्न** | ट्रांज़ैक्शन सिग्नेचर (str) |
...
| **पैरामीटर** | `sign_message`: वॉलेट साइन फ़ंक्शन (callable) |
|----------------|--------------------------|
| **रिटर्न** | `priv_key` (bytes) और `pub_key` (str) वाला dict |

```python
from iqlabs import crypto

# वॉलेट सिग्नेचर से एन्क्रिप्शन की-पेयर डेराइव करें
...
| **पैरामीटर** | `recipient_pub_hex`: प्राप्तकर्ता की पब्लिक की (हेक्स str)<br/>`plaintext`: एन्क्रिप्ट करने वाला डेटा (bytes) |
|----------------|--------------------------|
| **रिटर्न** | `sender_pub`, `iv`, `ciphertext` वाला dict |

#### `dh_decrypt()`

| **पैरामीटर** | `recipient_priv_key`: प्राप्तकर्ता की प्राइवेट की (bytes)<br/>`sender_pub`: भेजने वाले की पब्लिक की (हेक्स str)<br/>`iv`: इनिशियलाइज़ेशन वेक्टर (str)<br/>`ciphertext`: एन्क्रिप्टेड टेक्स्ट (str) |
|----------------|--------------------------|
| **रिटर्न** | bytes (प्लेनटेक्स्ट) |

```python
from iqlabs import crypto

# एकल प्राप्तकर्ता के लिए एन्क्रिप्ट करें
...
| **पैरामीटर** | `password`: पासवर्ड (str)<br/>`plaintext`: एन्क्रिप्ट करने वाला डेटा (bytes) |
|----------------|--------------------------|
| **रिटर्न** | `salt`, `iv`, `ciphertext` वाला dict |

#### `password_decrypt()`

| **पैरामीटर** | `password`: पासवर्ड (str)<br/>`salt`: साल्ट (str)<br/>`iv`: इनिशियलाइज़ेशन वेक्टर (str)<br/>`ciphertext`: एन्क्रिप्टेड टेक्स्ट (str) |
|----------------|--------------------------|
| **रिटर्न** | bytes (प्लेनटेक्स्ट) |
...
| **पैरामीटर** | `recipient_pub_hex_array`: प्राप्तकर्ताओं की पब्लिक कीज़ की सूची (list[str])<br/>`plaintext`: एन्क्रिप्ट करने वाला डेटा (bytes) |
|----------------|--------------------------|
| **रिटर्न** | मल्टी-प्राप्तकर्ता एन्क्रिप्टेड ऑब्जेक्ट |

#### `multi_decrypt()`

| **पैरामीटर** | `recipient_priv_key`: प्राप्तकर्ता की प्राइवेट की (bytes)<br/>`recipient_pub_hex`: प्राप्तकर्ता की पब्लिक की (हेक्स str)<br/>`encrypted`: मल्टी-प्राप्तकर्ता एन्क्रिप्टेड ऑब्जेक्ट |
|----------------|--------------------------|
| **रिटर्न** | bytes (प्लेनटेक्स्ट) |
...
# कई प्राप्तकर्ताओं के लिए एन्क्रिप्ट करें
encrypted = crypto.multi_encrypt([alice_pub_hex, bob_pub_hex, carol_pub_hex], b'group secret')

# प्राप्तकर्ताओं में से एक के रूप में डिक्रिप्ट करें
...
| **मॉड्यूल** | `writer` |
|------------|----------|
| **पैरामीटर** | `connection`: AsyncClient<br/>`signer`: Keypair या WalletSigner<br/>`db_root_id`: डेटाबेस ID (bytes या str)<br/>`seed`: टेबल या कनेक्शन सीड (bytes या str)<br/>`row_json`: JSON पंक्ति डेटा (str)<br/>`table_name`: टेबल एडिट के लिए आवश्यक (वैकल्पिक)<br/>`target_tx`: टेबल एडिट के लिए रेफ़रेंस tx (वैकल्पिक) |
| **रिटर्न** | ट्रांज़ैक्शन सिग्नेचर (str) |
