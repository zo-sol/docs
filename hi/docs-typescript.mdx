...
टेबल को स्पष्ट रूप से बनाने के लिए [`createTable()`](#createtable) का उपयोग करें। यदि DbRoot PDA में जगह कम है, तो SDK उसी ट्रांज़ैक्शन में स्वचालित रूप से इसे विस्तारित करता है — कोई अतिरिक्त कदम आवश्यक नहीं।
...
- [`createTable()`](#createtable): नई टेबल बनाएं (आवश्यकता होने पर DbRoot स्वचालित रूप से विस्तारित)
- [`writeRow()`](#writerow): मौजूदा टेबल में नई पंक्ति जोड़ें
...
वॉलेट सिग्नेचर से [`deriveX25519Keypair()`](#derivex25519keypair) के माध्यम से X25519 की-पेयर डेराइव किया जाता है।

#### संबंधित प्रमुख फ़ंक्शन्स

- [`deriveX25519Keypair()`](#derivex25519keypair): वॉलेट सिग्नेचर से एन्क्रिप्शन की-पेयर डेराइव करें
- [`dhEncrypt()`](#dhencrypt) / [`dhDecrypt()`](#dhdecrypt): एकल प्राप्तकर्ता एन्क्रिप्शन/डिक्रिप्शन
- [`passwordEncrypt()`](#passwordencrypt) / [`passwordDecrypt()`](#passworddecrypt): पासवर्ड-आधारित एन्क्रिप्शन/डिक्रिप्शन
- [`multiEncrypt()`](#multiencrypt) / [`multiDecrypt()`](#multidecrypt): मल्टी-प्राप्तकर्ता एन्क्रिप्शन/डिक्रिप्शन
...
| **पैरामीटर्स** | `connection`: Solana RPC कनेक्शन<br/>`signer`: साइनर<br/>`dbRootId`: डेटाबेस ID (Uint8Array या string)<br/>`meta`: स्टोर करने के लिए मेटाडेटा (Uint8Array या string) |
|----------------|--------------------------|
| **रिटर्न** | ट्रांजैक्शन सिग्नेचर (string) |
...
| **पैरामीटर** | `connection`: Solana RPC Connection<br/>`signer`: Signer<br/>`dbRootId`: डेटाबेस ID (Uint8Array या string)<br/>`tableSeed`: टेबल सीड (Uint8Array या string)<br/>`tableName`: टेबल नाम (Uint8Array या string)<br/>`columnNames`: कॉलम सूची (Array\<Uint8Array \| string\>)<br/>`idCol`: ID कॉलम (Uint8Array या string)<br/>`extKeys`: एक्सटेंशन कीज़ (Array\<Uint8Array \| string\>)<br/>`gateMint`: वैकल्पिक टोकन गेट (PublicKey)<br/>`writers`: वैकल्पिक अनुमत लेखक (PublicKey[]) |
|----------------|--------------------------|
| **रिटर्न** | ट्रांज़ैक्शन सिग्नेचर (string) |
...
| **पैरामीटर्स** | `connection`: Solana RPC कनेक्शन<br/>`signer`: साइनर<br/>`dbRootId`: डेटाबेस ID (Uint8Array या string)<br/>`tableSeed`: टेबल नाम (Uint8Array या string)<br/>`rowJson`: JSON पंक्ति डेटा (string)<br/>`skipConfirmation`: tx कन्फर्मेशन छोड़ें (boolean, डिफ़ॉल्ट: false)<br/>`remainingAccounts`: रीड-ओनली रेफरेंस अकाउंट के रूप में जोड़ने के लिए PublicKey की वैकल्पिक ऐरे (जैसे फीड PDA इंडेक्सिंग के लिए) |
|----------------|--------------------------|
| **रिटर्न** | ट्रांजैक्शन सिग्नेचर (string) |
...
| **पैरामीटर्स** | `signMessage`: वॉलेट साइन फ़ंक्शन `(message: Uint8Array) => Promise<Uint8Array>` |
|----------------|--------------------------|
| **रिटर्न** | `{ privKey: Uint8Array, pubKey: string }` |
...
| **पैरामीटर्स** | `recipientPubHex`: प्राप्तकर्ता की पब्लिक की (हेक्स string)<br/>`plaintext`: एन्क्रिप्ट करने वाला डेटा (Uint8Array) |
|----------------|--------------------------|
| **रिटर्न** | `{ senderPub: string, iv: string, ciphertext: string }` |

#### `dhDecrypt()`

| **पैरामीटर्स** | `recipientPrivKey`: प्राप्तकर्ता की प्राइवेट की (Uint8Array)<br/>`senderPub`: भेजने वाले की पब्लिक की (हेक्स string)<br/>`iv`: इनिशियलाइज़ेशन वेक्टर (string)<br/>`ciphertext`: एन्क्रिप्टेड टेक्स्ट (string) |
|----------------|--------------------------|
| **रिटर्न** | `Uint8Array` (प्लेनटेक्स्ट) |
...
| **पैरामीटर्स** | `password`: पासवर्ड (string)<br/>`plaintext`: एन्क्रिप्ट करने वाला डेटा (Uint8Array) |
|----------------|--------------------------|
| **रिटर्न** | `{ salt: string, iv: string, ciphertext: string }` |

#### `passwordDecrypt()`

| **पैरामीटर्स** | `password`: पासवर्ड (string)<br/>`salt`: साल्ट (string)<br/>`iv`: इनिशियलाइज़ेशन वेक्टर (string)<br/>`ciphertext`: एन्क्रिप्टेड टेक्स्ट (string) |
|----------------|--------------------------|
| **रिटर्न** | `Uint8Array` (प्लेनटेक्स्ट) |
...
| **पैरामीटर्स** | `recipientPubHexArray`: प्राप्तकर्ताओं की पब्लिक कीज़ की ऐरे (string[])<br/>`plaintext`: एन्क्रिप्ट करने वाला डेटा (Uint8Array) |
|----------------|--------------------------|
| **रिटर्न** | मल्टी-प्राप्तकर्ता एन्क्रिप्टेड ऑब्जेक्ट |

#### `multiDecrypt()`

| **पैरामीटर्स** | `recipientPrivKey`: प्राप्तकर्ता की प्राइवेट की (Uint8Array)<br/>`recipientPubHex`: प्राप्तकर्ता की पब्लिक की (हेक्स string)<br/>`encrypted`: मल्टी-प्राप्तकर्ता एन्क्रिप्टेड ऑब्जेक्ट |
|----------------|--------------------------|
| **रिटर्न** | `Uint8Array` (प्लेनटेक्स्ट) |
