...
| **Parameters** | `connection`: AsyncClient<br/>`signer`: Keypair or WalletSigner<br/>`db_root_id`: database ID (bytes or str)<br/>`table_seed`: table identifier — hashed internally via `to_seed_bytes()` for PDA derivation<br/>`table_name`: display name (bytes or str)<br/>`column_names`: column list (list[bytes or str])<br/>`id_col`: ID column (bytes or str)<br/>`ext_keys`: extension keys (list[bytes or str])<br/>`gate`: optional access gate (see [Token & Collection Gating](#token--collection-gating))<br/>`writers`: optional allowed writers (list[Pubkey])<br/>`table_hint`: human-readable identifier stored in DbRoot for discovery (str) |
|----------------|--------------------------|
| **Returns** | Transaction signature (str) |
